...
def test(fraction, lr, weight_mean=0, weight_variance=500, iter=10000, func=relu, func_der=relu_derivative, verbosity="low"):
    ...
    w = np.random.normal(0, weight_variance, (10, 64))
...
def main():
    ...
    for lr in range(-3, 15):
        for fraction in range(5, 32):
            print (lr, fraction)
            acc = test(fraction, lr, iter=100000, func=relu, func_der=relu_derivative, verbosity="none")
            accuracy.append((lr, fraction, acc))

    with open('digits.pickle', 'wb') as output:
        pickle.dump(accuracy, output, pickle.HIGHEST_PROTOCOL)
